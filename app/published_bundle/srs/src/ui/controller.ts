...
function discoverCardGroups(root: ParentNode, bundleGuid: string, pageId: string): SrsCardGroup[] {
...
export function initializeMeadowSrs(options: InitializeSrsOptions): MeadowSrsController {
  ...
  const groups = discoverCardGroups(root, options.bundleGuid, options.pageId);
  const definitions = groups.flatMap((group) => group.definitions);
  const store = loadStore(options.bundleGuid, persistence);
  ...
  let reviewScope: 'page' | 'bundle' = 'page';
  ...
  const applyReview = (runtimeCard: SrsRuntimeCard, rating: SrsReviewRating) => {
    ...
    saveStore(options.bundleGuid, persistence, store);
  ...
  const renderCard = (runtimeCard: SrsRuntimeCard, overlayMode: boolean = false): HTMLElement => {
    ...
    if (overlayMode && reviewScope === 'bundle' && runtimeCard.definition.contextPath.length > 0) {
  ...
  const renderOverlayCard = () => {
    ...
    let bundleDueCount: number | null = null;
    let bundleNewCount: number | null = null;
    if (globalDefinitions !== null) {
      const pageCardIds = new Set(definitions.map(d => d.id));
      const allDefinitions = [...definitions, ...globalDefinitions.filter(d => !pageCardIds.has(d.id))];
      const bundleRuntimeCards = getRuntimeCards(allDefinitions, store, clock);
      bundleDueCount = bundleRuntimeCards.filter((card) => !card.buried && card.due && !card.newCard).length;
      bundleNewCount = bundleRuntimeCards.filter((card) => !card.buried && card.newCard).length;
    }
    ...
    overlay.querySelectorAll<HTMLButtonElement>('.meadow-srs-overlay__tab').forEach((tab) => {
      ...
      const dueCount = tab.dataset.scope === 'page' ? pageDueCount : bundleDueCount;
      const newCount = tab.dataset.scope === 'page' ? pageNewCount : bundleNewCount;
      if (dueCount !== null) {
    ...
    if (reviewCards.length === 0) {
      if (reviewMode === 'due') {
        ...
        subtitle.textContent = reviewScope === 'page'
          ...
          : 'No prompts found across the bundle.';
      }
  ...
  return {
    ...
    refresh: rerender,
    clearState: () => {
      clearStore(options.bundleGuid, persistence);
      const empty = loadStore(options.bundleGuid, persistence);
...
export function autoInitializeMeadowSrs(): MeadowSrsController | null {
  const bundleGuid = document.body.dataset.meadowSrsBundleGuid;
  const pageId = document.body.dataset.meadowSrsPageId;
  if (!bundleGuid || !pageId) {
    return null;
  }
  return initializeMeadowSrs({ bundleGuid, pageId });
}
