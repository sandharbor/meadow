...
describe('Pagespecs Curation System Tests', () => {
  ...
  describe('Link Spec Validation Tests', () => {
    ...
    it('validateLinkSpec should reject invalid link specs', () => {
      expect(validateLinkSpec({ isInGraph: true }, 'test', 'testPage', 'testSite').length).toBeGreaterThan(0);
      expect(validateLinkSpec({ linkPath: 'invalid', isInGraph: true }, 'test', 'testPage', 'testSite').length).toBeGreaterThan(0);
      expect(validateLinkSpec({ linkPath: '/page.md' }, 'test', 'testPage', 'testSite').length).toBeGreaterThan(0);
  ...
  describe('Links Section Validation Tests', () => {
    it('validateLinksSection should accept valid links sections', () => {
      ...
      }, 'testPage', 'testSite')).toHaveLength(0);
    ...
    it('validateLinksSection should reject invalid links sections', () => {
      ...
      expect(validateLinksSection({ outlinks: 'not an array' }, 'testPage', 'testSite').length).toBeGreaterThan(0);
      expect(validateLinksSection({ inlinks: 'not an array' }, 'testPage', 'testSite').length).toBeGreaterThan(0);
    });
  });
...
async function validatePagespecLinksForSite(
  ...
  for (const mdFile of mdFiles) {
    ...
    if (!siteSpec.curation.links) continue;

    const result = checkPagespecLinks(
      siteSpec.curation.links,
      pageTitle,
      {
  ...
  it('should validate isInWorkingGraph claims for pages declaring false', async () => {
    ...
    for (const { name: siteName, setup: siteSetup, initialPage, sourceGraphDir } of configuredSites) {
      ...
      for (const mdFile of mdFiles) {
        ...
        const siteSpec = getPagespecForSite(block, siteName);
        if (!siteSpec || siteSpec.curation.isInWorkingGraph !== false) continue;
  ...
  it('should validate frontierDepthOrNullForOrphan matches actual frontier depth', async () => {
    ...
    for (const { name: siteName, setup: siteSetup, initialPage, sourceGraphDir } of configuredSites) {
      ...
      for (const mdFile of mdFiles) {
        ...
        const expectedFrontierDepth = siteSpec.curation.frontierDepthOrNullForOrphan;
