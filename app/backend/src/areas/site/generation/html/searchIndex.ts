...
export interface PublishedSiteSearchDocument {
  /** Normalized title shown on the generated page. */
  t: string;
  /** URL-encoded path relative to the generated-site root. */
  p: string;
...
): PublishedSiteSearchDocument | null {
...
export function collectPublishedSiteSearchDocuments(
  generatedSiteDirectory: string
): PublishedSiteSearchDocument[] {
  return walkHtmlFiles(generatedSiteDirectory)
    .map(filePath => {
      const relativeHtmlPath = path.relative(generatedSiteDirectory, filePath);
      return searchDocumentFromHtml(fs.readFileSync(filePath, 'utf8'), relativeHtmlPath);
    })
    .filter((document): document is PublishedSiteSearchDocument => document !== null)
...
export function copyPublishedSiteSearchAssets(
...
export function writePublishedSiteSearchIndex(
  generatedSiteDirectory: string,
  generatedAssetsDirectory: string
): { documentCount: number; shardCount: number } {
  const documents = collectPublishedSiteSearchDocuments(generatedSiteDirectory);
  const shards = new Map<string, PublishedSiteSearchDocument[]>();
