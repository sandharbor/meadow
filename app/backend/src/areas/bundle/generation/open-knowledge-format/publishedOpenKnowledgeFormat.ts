...
export async function generatePublishedOpenKnowledgeFormatArtifacts(
  ...
  const okfDir = BundleConfigPaths.getOpenKnowledgeFormatDir(options.bundleDirectory);
  ...
  writeOpenKnowledgeFormatGenerationManifest(options.bundleDirectory, result);
