...
/**
 * Get the file tree for a bundle's config directory.
 * @param changedOnly - If true, only return files with git changes (much faster for large directories)
 */
export async function getConfigFileTree(bundleDirectory: string, changedOnly = false): Promise<FileTreeResponse> {
  const configDir = path.join(bundleDirectory, 'config');
  const gitStatusMap = await getGitStatusMap(configDir);
  
  // Use optimized path when only showing changed files
  const tree = changedOnly 
    ? buildChangedFilesTree(configDir, gitStatusMap)
    : buildFileTree(configDir, gitStatusMap);
