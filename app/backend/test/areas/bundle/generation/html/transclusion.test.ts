...
describe('html transclusion', () => {
  it('full page transclusion should embed entire page content', () => {
    ...
    try {
      ...
      const bundleNodeConfigs: BundleNodeConfig[] = [
        makeBundleNodeConfig('t017 ---- full page source', 'whitelist', { sourceGraphSubdirectory: 't017' }),
      ];
  ...
  it('section transclusion should embed only the requested section', () => {
    ...
    try {
      ...
      const bundleNodeConfigs: BundleNodeConfig[] = [
        makeBundleNodeConfig('t017 ---- section source', 'whitelist', { sourceGraphSubdirectory: 't017' }),
      ];
  ...
  it('block transclusion should embed only the block containing the requested ^id', () => {
    ...
    try {
      ...
      const bundleNodeConfigs: BundleNodeConfig[] = [
        makeBundleNodeConfig('t017 ---- block source', 'whitelist', { sourceGraphSubdirectory: 't017' }),
      ];
  ...
  it('nested transclusion should only show one see-in-context button (top-level only)', () => {
    ...
    try {
      ...
      const bundleNodeConfigs: BundleNodeConfig[] = [
        makeBundleNodeConfig('outer', 'whitelist', { sourceGraphSubdirectory: 't017' }),
        makeBundleNodeConfig('inner', 'whitelist', { sourceGraphSubdirectory: 't017' }),
      ];
  ...
  it('link resolution inside transcluded content should use the transcluded page map but compute URLs relative to the final page', () => {
    ...
    try {
      ...
      const bundleNodeConfigs: BundleNodeConfig[] = [
        makeBundleNodeConfig('source', 'whitelist', { sourceGraphSubdirectory: 'a' }),
        makeBundleNodeConfig('dest', 'whitelist', { sourceGraphSubdirectory: 'b' }),
      ];
