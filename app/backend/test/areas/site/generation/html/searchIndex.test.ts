...
import {
  collectPublishedSiteSearchDocuments,
  searchDocumentFromHtml,
  searchShardId,
  writePublishedSiteSearchIndex,
} from '../../../../../src/areas/site/generation/html/searchIndex.js';

describe('published site search index', () => {
  ...
  it('ignores HTML outside rendered page content and generated assets', () => {
    const siteDirectory = makeTempDirectory();
    fs.mkdirSync(path.join(siteDirectory, 'nested'), { recursive: true });
    fs.mkdirSync(path.join(siteDirectory, '_mw_assets'), { recursive: true });
    fs.writeFileSync(path.join(siteDirectory, 'Page one.html'), pageHtml('Page one', 'first body'));
    fs.writeFileSync(path.join(siteDirectory, 'nested', 'Page two.html'), pageHtml('Page two', 'second body'));
    fs.writeFileSync(path.join(siteDirectory, '_mw_assets', 'ignored.html'), pageHtml('Ignored', 'asset body'));

    const documents = collectPublishedSiteSearchDocuments(siteDirectory);
  ...
  it('changes only the stable page shard when page content changes', () => {
    const siteDirectory = makeTempDirectory();
    const assetsDirectory = path.join(siteDirectory, '_mw_assets');
    ...
    fs.writeFileSync(path.join(siteDirectory, 'Page one.html'), pageHtml('Page one', 'changed first body'));
    writePublishedSiteSearchIndex(siteDirectory, assetsDirectory);
    const after = new Map(
      fs.readdirSync(indexDirectory).map(filename => [
        filename,
