...
describe('tracked page content for folder-derived sites', () => {
  const setup = new TestSiteSetup(
    '../../shared_data/home_fixtures/home_fixture_folder_structure_single/sites/single-folder-site',
    'single-folder-site',
  );
  ...
  it('materializes selected-folder descendants and linked pages for preview generation', async () => {
    const persistedConfigPath = SiteConfigPaths.getSiteNodeConfigFile(sitePath);
    const persistedConfigBefore = fs.readFileSync(persistedConfigPath, 'utf8');

    await ensureTrackedPageContent(sitePath, sourceGraphDirectory);

    expect(fs.readFileSync(persistedConfigPath, 'utf8')).toBe(persistedConfigBefore);
    const trackedConfigPath = SiteConfigPaths.getTrackedSiteNodeConfigFile(sitePath);
    const trackedConfigBefore = fs.readFileSync(trackedConfigPath, 'utf8');
    const trackedConfigs = parseSiteNodeConfig(trackedConfigBefore, trackedConfigPath);
    expect(trackedConfigs).toHaveLength(7);
    expect(trackedConfigs.map(config => config.siteNodeName)).toEqual(expect.arrayContaining([
      'Alpha', 'Alpha note', 'Nested note', 'Beyond outside', 'Outside note', 'Nested', 'Visual map',
    ]));
    const derivedByName = new Map(trackedConfigs.slice(1).map(config => [config.siteNodeName, config]));
    ...
    const trackedContent = SiteConfigPaths.getTrackedPageContentDir(sitePath);
