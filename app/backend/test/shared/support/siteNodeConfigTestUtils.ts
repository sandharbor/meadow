...
export function makeSiteNodeConfig(
  siteNodeName: string,
  ...
  const siteNodeId = crypto.createHash('sha256')
    .update([siteNodeName, sourceGraphSubdirectory, fileType].join('\0'))
    .digest('hex')
    .slice(0, 12) as SiteNodeId;
  return {
    siteNodeName,
