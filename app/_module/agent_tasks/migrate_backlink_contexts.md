...
Pagespec entries are organized by system area. The backlink context expectation
belongs to the generation section:
...
- Type definitions: `app/system_tests/pagespecs/types.ts`
- Structural validation: `app/system_tests/pagespecs/pagespecValidation.ts`
- HTML extractor: `app/system_tests/helpers/htmlLinkExtractor.ts`
- Runtime generation test:
  `app/system_tests/tests/areas/site/generation/pagespecs_generation.test.ts`
...
Run the pagespec generation test and look for the first "Missing
backlinkContexts" error:

```bash
cd app && npx vitest run system_tests/tests/areas/site/generation/pagespecs_generation.test.ts 2>&1
```
...
to add the `backlinkContexts` array under
`generation.htmlRenderedLinks.footerSectionBacklinks`. Update **all** backlink
entries for the page, not just the one reported in the error.
