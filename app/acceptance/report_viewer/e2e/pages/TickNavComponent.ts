...
export class TickNavComponent {
  ...
  private get headerTickButton() {
    // The header button starts with "Tick:" before selection and its
    // selected-state summary starts with the lowercase "tick N" chip.
    return this.page.getByRole("button", { name: /^(?:Tick:|tick \d+)/ });
  }
   ...
   * Verifies the header label updates to the selected tick before returning.
   */
  async goToTick(tickNumber: number): Promise<void> {
    ...
    await item.click();
    await this.expect(
      this.page.getByRole("button", { name: new RegExp(`^tick ${tickNumber}\\b`) }),
    ).toBeVisible();
  }
}
