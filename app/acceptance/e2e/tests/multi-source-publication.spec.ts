...
test('Multi-source publication retains old pages and connects their stable identities through a source rename', async ({ page, testServer, minioS3, addKeyFrame, checkpoint, skipMeadowHomeStateCheck }) => {
  ...
  await checkpoint('namesake pages and assets have distinct published source paths');
  ...
  await checkpoint('source settings explain publication path changes before saving the rename');
  ...
  await checkpoint('a connected successor is published while its predecessor remains available');
  ...
  await addKeyFrame(publicationRevision);
  await checkpoint('the retained old page offers its stable counterpart at the new source path');
  ...
  await reader.expectMainHeadingVisible('Overview');
  await addKeyFrame(publicationRevision);
  await checkpoint("the newer version opens the same page at its moved source path");
