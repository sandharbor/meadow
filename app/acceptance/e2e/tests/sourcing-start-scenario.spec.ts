...
test('Sourcing Start scenario resets the fixture and hands over directly in source review', async ({ page, addKeyFrame, skipMeadowHomeStateCheck }, testInfo) => {
  ...
  try {
    ...
    await expect(change.getByRole('button', { name: 'Apply change', exact: true })).toBeDisabled();
    await addKeyFrame(sourceChange);
    await change.getByRole('button', { name: 'Start scenario', exact: true }).click();
    await expect(page.getByRole('status').filter({ hasText: 'is ready to review' })).toBeVisible({ timeout: 120000 });
