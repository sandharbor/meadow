 ...
 */
test('Sourcing acceptance leaves filter-sensitive additions untracked and shows exactly the skipped pages', async ({ page, sourceChanges, snapshot, addKeyFrame, skipMeadowHomeStateCheck }) => {
  // --- Setup ---
  ...
  await snapshot('the sensitivity filter and earlier untracked addition are ready');
  ...
  await snapshot('the filter-sensitive additions remain untracked while the safe peer is tracked');
