...
`acceptance/e2e/_module/docs/coding_standards.md`. You may also refactor shared code
...
List all `.spec.ts` files in `acceptance/e2e/tests/`.
...
```bash
python3 -c "import random, glob; files = sorted(glob.glob('acceptance/e2e/tests/*.spec.ts')); print(files[random.randint(0, len(files)-1)])"
```

### 3. Review against coding standards

Read `acceptance/e2e/_module/docs/coding_standards.md` and then read the selected
...
- **Use workflows for navigation** — Is navigation inlined when a `Workflows`
  method exists or should be created? Read `acceptance/e2e/src/run/workflows.ts` to
  check.
- **Page objects own their selectors** — Are there locator strings duplicated
  ...
  objects in `acceptance/e2e/src/run/pages/`.
