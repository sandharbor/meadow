...
```bash
cd acceptance/e2e && LATEST_RUN=$(ls -t ~/meadow-e2e-artifacts/current/ | head -1) && npx tsx src/cli.ts --status "$LATEST_RUN"
```
...
1. Read the test file in `acceptance/e2e/tests/`.
...
3. Read the relevant page objects in `acceptance/e2e/src/run/pages/` and
   workflows in `acceptance/e2e/src/run/workflows.ts` to understand what
