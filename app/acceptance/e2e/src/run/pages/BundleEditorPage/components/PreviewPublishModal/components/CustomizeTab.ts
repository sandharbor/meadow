...
class Hook {
  ...
  /** The Bundle column button (last button in the row). */
  private get bundleCell() {
  ...
  /** Click the Bundle column cell to open the bundle hook editor (creates with template if new). */
  async clickCreate() {
    const labelEl = this.page.getByText(this.label, { exact: true });
    await labelEl.scrollIntoViewIfNeeded();
    await this.bundleCell.click();
  }
...
class GenerationOptionsSection {
  ...
  /** Get the Bundle column HoverSelect combobox for a named setting row. */
  private settingBundleCombobox(name: string) {
    const row = this.settingRow(name);
    // Bundle column is the last combobox in the row (Global is first)
  ...
  private async selectHoverOption(name: string, optionLabel: string) {
    const combobox = this.settingBundleCombobox(name);
  ...
  /** Set the Breadcrumbs bundle-level setting to "Off" (disabled). */
  async disableBreadcrumbs() {
    await this.selectHoverOption("Breadcrumbs", "Off");
  }

  /** Set the Backlinks bundle-level setting to "Off" (disabled). */
