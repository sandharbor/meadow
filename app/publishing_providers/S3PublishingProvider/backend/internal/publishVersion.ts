...
export async function publishS3Version(options: PublishGeneratedBundleOptions) {
  const { bundleSlug, operationId } = options;
  const operation = `[operation ${operationId}] [s3-publish]`;
  if (!bundleSlug) throw new PublishingProviderOperationError({ statusCode: 400, code: 'PUBLICATION_REJECTED', message: 'bundleSlug is required' });
  const bundleDirectory = getBundleDirectory(bundleSlug);
  if (!fs.existsSync(bundleDirectory)) throw new PublishingProviderOperationError({ statusCode: 404, code: 'PUBLICATION_REJECTED', message: `Bundle '${bundleSlug}' not found` });

  let versionId;
  try {
    versionId = requireGeneratedBundleVersionId(options.versionId);
  } catch (error) {
    throw new PublishingProviderOperationError({ statusCode: 400, code: 'PUBLICATION_REJECTED', message: error instanceof Error ? error.message : String(error) });
  }
  ...
  if (!localEntry || localEntry.localFilesState === 'deleted') {
    throw new PublishingProviderOperationError({ statusCode: 400, code: 'PUBLICATION_REJECTED', message: 'The selected version is not locally present' });
  }
  try {
    assertFrozenGeneratedVersionsIntegrity(bundleDirectory, localManifest);
  } catch (error) {
    throw new PublishingProviderOperationError({ statusCode: 409, code: 'PUBLICATION_REJECTED', message: error instanceof Error ? error.message : String(error) });
  }
  const gitState = inspectGeneratedVersionGitState(bundleDirectory, versionId);
  if (!gitState.isSaved || !gitState.savedGenerationId) {
    throw new PublishingProviderOperationError({ statusCode: 409, code: 'GENERATION_NOT_SAVED', message: 'Save the selected generated version before publishing' });
  }
  ...
  if (!bundleConfig.publishSlug) {
    throw new PublishingProviderOperationError({ statusCode: 400, code: 'PUBLICATION_REJECTED', message: 'publishSlug is not set. Open the Publish tab and set it before publishing.' });
  }
  ...
  try {
    ...
    throw new PublishingProviderOperationError({ statusCode: 500, code: 'PUBLICATION_REJECTED', message: error instanceof Error ? error.message : String(error) });
  }
  ...
  if (s3DestinationFieldsLocked(loadedState, { publishSlug, bucketName: bucket })) {
    throw new PublishingProviderOperationError({ statusCode: 409, code: 'PUBLICATION_REJECTED', message: 'S3 destination fields cannot change while remote versions remain' });
  }
  ...
  if (!revision) throw new PublishingProviderOperationError({ statusCode: 500, code: 'PUBLICATION_REJECTED', message: 'Publication revision planning failed' });
  ...
  if (!secrets.s3AccessKeyId || !secrets.s3SecretAccessKey) {
    throw new PublishingProviderOperationError({ statusCode: 400, code: 'PUBLICATION_REJECTED', message: 'No S3 credentials are configured. Set them under S3 configuration.' });
  }
  ...
  try {
    ...
    return {
      success: true,
      providerInstanceId: stateBefore.providerInstanceId,
      versionId,
      savedGenerationId,
      publishedUrl,
    ...
    throw new PublishingProviderOperationError({
      ...
      message: `${describeS3Error(error)} No publication success was recorded; retry is safe.`,
    });
  }
}
