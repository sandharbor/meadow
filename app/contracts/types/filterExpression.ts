...
/** Visibility-expression primitives shared by every Meadow graph client. */
...
export function evaluateFilterExpression(
  ...
  const evaluate = (node: FilterExpression): Set<string> | null => {
    ...
    if (node.type === 'filter') {
      ...
      return node.mode === 'solo'
        ? matches
        : new Set([...allBundleNodeKeys].filter(bundleNodeKey => !matches.has(bundleNodeKey)));
    }
  ...
  return evaluate(expression) || new Set(allBundleNodeKeys);
}
