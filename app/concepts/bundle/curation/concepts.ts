...
export const paths = defineMeadowConcept({
  ...
  mechanics: [
    conceptText`Path-oriented workflows select relevant routes and expose the sequence of relationships leading to a node. Each step's depth, inherited budgets, overrides, and remaining budgets describe its arrival along the displayed path. An intermediate page may have a shorter independent arrival with less remaining budget; its own shortest path must not replace the arrival that enabled traversal to the selected endpoint. The route diagram shows the shortest arrival and the other useful budget combinations, retaining their paired budgets and exact steps. When an override makes arrivals equivalent for traversal, a route supplying the largest inherited budget in each overridden direction remains available to explain what was replaced. Such explanatory arrivals never expand traversal or change remaining-depth summaries. Route indicators distinguish arrivals retained for traversal from explanation-only arrivals using recorded provenance, not numeric maxima or which route the user selected. Multiple independent budget pairs may be retained; older snapshots with no provenance status do not infer it. Simple depth summaries show independent maxima across arrivals. These maxima do not form a combined traversal state: outlink/inlink arrivals of 5/0 and 2/3 do not permit traversal as 5/3.`,
  ],
