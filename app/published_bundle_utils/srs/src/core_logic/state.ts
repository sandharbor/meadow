...
export function loadStore(bundleGuid: string, persistence: SrsPersistence): SrsStoreData {
  const raw = persistence.load(makeStorageKey(bundleGuid));
  if (!raw) {
    return emptyStore();
  }
...
export function saveStore(bundleGuid: string, persistence: SrsPersistence, store: SrsStoreData): void {
  persistence.save(makeStorageKey(bundleGuid), JSON.stringify(store));
}

export function clearStore(bundleGuid: string, persistence: SrsPersistence): void {
  persistence.clear(makeStorageKey(bundleGuid));
}
