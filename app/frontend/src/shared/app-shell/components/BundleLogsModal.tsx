...
interface BundleLogsModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialBundleGuidFilter?: string | null;
}

const BundleLogsModal: React.FC<BundleLogsModalProps> = ({ isOpen, onClose, initialBundleGuidFilter }) => {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const initial = (initialBundleGuidFilter || '').trim();
    ...
    setLines([]);
    setError(null);
  }, [isOpen, initialBundleGuidFilter]);
      ...
      <div className="flex flex-col h-full min-h-0">
        <div className="flex items-end gap-2 mb-3">
          <div className="flex flex-col">
            <label className="text-xs text-neutral-600">Bundle filter (optional)</label>
            <input
              value={draftFilter}
              ...
              placeholder="e.g. [bundle a1b2c3d]"
            />
          </div>
          <button
