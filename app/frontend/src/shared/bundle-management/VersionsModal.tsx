...
const VersionsModal: React.FC<VersionsModalProps> = ({ 
      ...
      if (response.ok) {
        ...
        const versionsWithUrls = await Promise.all(
          (data.versions || []).map(async (version: Version) => {
            ...
            try {
              const url = await fetchUrl(bundleSlug, version.versionId);
              return { ...version, url };
            } catch (err) {
  ...
  }, [isOpen, bundleSlug, loadVersions]);
  ...
  const handleCreateNewVersion = async () => {
    ...
    setPublishing(true);
    try {
      await provider.publishNewVersion(bundleSlug, newVersionNotes, addPointersToOlderVersions);
      await loadVersions();
  ...
  const handleSetActiveVersion = async (versionId: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/bundles/${bundleSlug}/review/versions/${versionId}/set-active`, {
        method: 'POST'
      });
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-2 flex-1">
                ...
                <div className="text-sm text-blue-800">
                  ...
                  <p>If you&rsquo;ve published and shared your bundle, creating a new version preserves existing URLs while allowing you to make significant changes. This ensures shared links continue to work while you iterate on your bundle.</p>
                </div>
              </div>
              {!openedFromPreview && (
        ...
        <div className="space-y-4">
          <p className="text-gray-700">
            {openedFromPreview 
              ...
              : "This will create a new version of your bundle and publish it to the web."
            }
          </p>
