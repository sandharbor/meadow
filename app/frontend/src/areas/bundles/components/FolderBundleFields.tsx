...
export interface FolderBundlePreflight {
  fingerprint: string;
  plan: {
...
interface FolderBundleFieldsProps {
  ...
  preflight: FolderBundlePreflight | null;
...
const FolderBundleFields: React.FC<FolderBundleFieldsProps> = ({
  ...
  <>
      ...
      <div>
        ...
        {selectedFolders.length === 0 ? (
          <p className="p-3 text-sm text-gray-500 border border-dashed border-gray-300 rounded-md">
            Choose folders beneath the source directory. Their order becomes the bundle-home order.
          </p>
        ) : (
    ...
    {preflight && (
      <section className="p-3 bg-blue-50 border border-blue-200 rounded-md" aria-label="Folder bundle prediction">
