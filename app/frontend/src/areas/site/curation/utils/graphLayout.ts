  ...
  sortedLevels.forEach((levelNodes, levelIndex) => {
    ...
    const place = (nodesAtLevel: GraphLayoutNode[], y: number): void => {
      ...
      nodesAtLevel.forEach((node, index) => {
        positions.set(node.siteNodeKey, {
...
function placeIndentedRow(
  ...
  nodes.forEach((node, index) => {
    positions.set(node.siteNodeKey, { x: startX + step * index, y });
  });
}
...
function semanticPredecessorAverage(
  ...
  for (const edge of semanticEdges) {
    ...
    if (edge.source === node.siteNodeKey) neighborKey = edge.target;
    if (edge.target === node.siteNodeKey) neighborKey = edge.source;
...
function orderSemanticRow(
  ...
  return [...row.nodes].sort((left, right) => {
    ...
    if (leftAverage === undefined && rightAverage !== undefined) return 1;
    return left.siteNodeName.localeCompare(right.siteNodeName)
      || left.siteNodeKey.localeCompare(right.siteNodeKey);
  });
}
...
export function calculateGraphLayout(
  ...
  const structuralEdges = edges.filter(edge => edge.siteEdgeKind !== 'semanticLink');
  const hasStructuralContainers = nodes.some(node => node.siteNodeKind !== 'file');
  ...
  const roots = nodes.filter(node =>
    node.siteNodeKind !== 'file' && !incomingStructural.has(node.siteNodeKey)
  );
  ...
  const visitContainer = (node: GraphLayoutNode, indent: number): void => {
    if (structurallyPlaced.has(node.siteNodeKey)) return;
    structurallyPlaced.add(node.siteNodeKey);
    folderRows.push({ kind: 'container', nodes: [node], indent });

    const children = (structuralChildren.get(node.siteNodeKey) ?? [])
      .map(key => nodesByKey.get(key))
      .filter((child): child is GraphLayoutNode => Boolean(child));
    const directFiles = children.filter(child => child.siteNodeKind === 'file');
    for (const file of directFiles) structurallyPlaced.add(file.siteNodeKey);
    for (const chunk of chunksForRow(directFiles, indent + 1, width)) {
      folderRows.push({ kind: 'files', nodes: chunk, indent: indent + 1 });
    }
    for (const child of children) {
      if (child.siteNodeKind !== 'file') visitContainer(child, indent + 1);
    }
  };
  ...
  for (const node of nodes) {
    if (node.siteNodeKind !== 'file' && !structurallyPlaced.has(node.siteNodeKey)) {
      visitContainer(node, 0);
    }
  }

  const semanticNodes = nodes.filter(node =>
    !structurallyPlaced.has(node.siteNodeKey) && (includeHidden || node.isVisible)
  );
  ...
  const semanticEdges = edges.filter(edge => edge.siteEdgeKind === 'semanticLink');
