...
import type { INormalSiteNodeSelector } from './filterSelectors.js';
...
interface NodeTypeFilterDefinition {
  id: NodeTypeFilterId;
  label: string;
  createSelector: () => INormalSiteNodeSelector;
}
...
const NODE_TYPE_FILTER_DEFINITIONS: NodeTypeFilterDefinition[] = [
  ...
  { id: 'folder', label: 'Folder Nodes', createSelector: () => createSiteNodeKindSelector('folder') },
  { id: 'collection', label: 'Site Homes', createSelector: () => createSiteNodeKindSelector('collection') },
...
export const createNodeTypeFilterSelector = (id: NodeTypeFilterId): INormalSiteNodeSelector => {
