...
// Only true when the bundle is in its initial state (only the traversal-start
...
export function useHasFrontierOutlinks(graph: Graph, graphUpdateTrigger?: number): boolean {
  return React.useMemo(() => {
    ...
    // Check that all other nodes are not tracked (bundle is in initial state).
    const otherTrackedNodes = allNodes.filter(
      node => node.depth !== 0 && node.tracked
    );
    if (otherTrackedNodes.length > 0) return false;

    const workingGraphNodeKeys = new Set<string>(allNodes.map(node => node.bundleNodeKey));
    ...
    for (const leafNode of leafNodes) {
      const outlinks = graph.getAllOutlinkTargets(leafNode.bundleNodeKey);
      ...
      <div className="bg-main-50 border border-main-200 rounded-lg shadow-lg p-3 relative">
        ...
        <div className="text-xs text-main-700 pr-4">
          ...
          <p className="text-main-600 mt-2">
            You can make your bundle bigger by selecting this page and increasing the outlink depth.
          </p>
        </div>
