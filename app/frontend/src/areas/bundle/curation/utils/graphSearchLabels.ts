...
export function computeLabelPlacements(
  ...
  const items = nodes.map(p => ({
    ...p,
    segments: splitTitleBySearch(p.bundleNodeName, searchText),
    width: p.bundleNodeName.length * charWidth + padding * 2,
    height: labelHeight,
  }));
  ...
  for (const item of items) {
    const { bundleNodeKey, nodeX, nodeY, segments, width, height } = item;
