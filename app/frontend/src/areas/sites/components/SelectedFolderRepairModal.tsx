...
const SelectedFolderRepairModal: React.FC<SelectedFolderRepairModalProps> = ({
  ...
  useEffect(() => {
    if (!isOpen) return;
    setTargetId(missingFolders[0]?.siteNodeId ?? '');
    setSelectedFolder('');
    setPreflight(null);
  ...
  const target = missingFolders.find(folder => folder.siteNodeId === targetId);

  const browse = async () => {
    const result = await window.electronAPI?.showOpenDialog({
      properties: ['openDirectory'],
      title: `Relink ${target?.siteNodeName ?? 'selected folder'}`,
  ...
  const review = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/sites/${encodeURIComponent(siteSlug)}/folders/relink-preflight`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ siteNodeId: targetId, selectedFolder }),
      });
  ...
  const commit = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/sites/${encodeURIComponent(siteSlug)}/folders/relink`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          siteNodeId: targetId,
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Relink selected folder" className="w-full max-w-2xl h-auto">
      <div className="space-y-4 p-1">
        <p className="text-sm text-neutral-700">
          Meadow cannot open this site until every selected folder is repaired. Choose an existing folder beneath
          <span className="font-mono"> {sourceDirectory}</span>. Meadow preserves the folder identity, site-home order, and role references.
        </p>
          ...
          <label className="block text-sm font-medium text-neutral-700">
            ...
            >
              {missingFolders.map(folder => (
                <option key={folder.siteNodeId} value={folder.siteNodeId}>{folder.sourceGraphSubdirectory || '(source root)'}</option>
              ))}
            </select>
          </label>
        )}
        {target && (
          <div className="rounded border border-danger-200 bg-danger-50 p-3 text-sm text-danger-800">
            Missing {target.role === 'entry' ? 'entry folder' : 'site-home folder'}: <span className="font-mono">{target.sourceGraphSubdirectory || '(source root)'}</span>
          </div>
        )}
        ...
        {preflight && (
          <div className="rounded border border-main-200 bg-main-50 p-3 text-sm text-neutral-800">
            ...
            <p className="mt-1 text-xs">Stable ID {preflight.preservedSiteNodeId}{preflight.collectionMemberIndex !== undefined ? ` · site-home position ${preflight.collectionMemberIndex + 1}` : ''}</p>
