...
import type { FindInSitesOptions } from '../../../../../shared_code/types/findInSitesOptions';
...
export type SiteModalMode = 'create' | 'edit';

export interface EditSiteDefaults {
  slug: string;
  sourceDirectory: string;
  entrySiteNodeName: string;
  entrySourceGraphSubdirectory?: string;
...
export interface CreateOrEditSiteModalProps {
  isOpen: boolean;
  onClose: () => void;
  mode: SiteModalMode;
...
export const EntryStrategyPicker: React.FC<EntryStrategyPickerProps> = ({ value, onChange }) => (
  <fieldset>
    <legend className="block text-sm font-medium text-gray-700 mb-2">Start this site from</legend>
    <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="Site entry strategy">
      {([
        ['page', 'A page', 'Use one source page as the site entry.'],
        ['folders', 'One or more folders', 'Build a curated site from recursive folder contents.'],
...
interface MoreSiteDetailsProps {
  expanded: boolean;
  isCreate: boolean;
  slug: string;
  notes: string;
...
export const MoreSiteDetails: React.FC<MoreSiteDetailsProps> = ({
  expanded,
  isCreate,
  slug,
  notes,
  ...
  <>
    ...
    {expanded && (
      <div className="space-y-4 pl-4 border-l-2 border-gray-200">
        {isCreate && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Site Config Folder Name *</label>
            {!isSlugManuallyEdited ? (
              <div className="flex items-center space-x-2">
        ...
        <div>
          ...
          <textarea value={notes} onChange={event => onNotesChange(event.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500" placeholder="Enter any notes about this site..." />
        </div>
      </div>
    )}
  </>
