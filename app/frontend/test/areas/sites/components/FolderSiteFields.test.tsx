...
import FolderSiteFields, {
  type FolderSitePreflight,
} from '../../../../src/areas/sites/components/FolderSiteFields';
...
describe('FolderSiteFields', () => {
  it('exposes ordered selection controls with unambiguous accessible names', () => {
    ...
    const list = screen.getByRole('list', { name: 'Selected folders in site-home order' });
  ...
  it('renders exact preflight evidence and requires explicit high-impact confirmation', () => {
    const handlers = callbacks();
    const preflight: FolderSitePreflight = {
      fingerprint: 'fingerprint',
      plan: {
        sourceDirectory: '/vault',
        normalizedSelectedFolders: ['Alpha'],
        folderSiteNodeIds: ['aaaaaaaaaaaa'],
        entrySiteNodeId: 'aaaaaaaaaaaa',
    ...
    const prediction = screen.getByRole('region', { name: 'Folder site prediction' });
