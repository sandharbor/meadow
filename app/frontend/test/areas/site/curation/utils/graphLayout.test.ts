...
} from '../../../../../src/areas/site/curation/utils/graphLayout';

const node = (
  siteNodeKey: string,
  siteNodeKind: GraphLayoutNode['siteNodeKind'],
  distance: number | undefined,
  siteNodeName = siteNodeKey,
): GraphLayoutNode => ({
  siteNodeKey,
  siteNodeKind,
  siteNodeName,
...
const edge = (
  source: string,
  target: string,
  siteEdgeKind: IEdge['siteEdgeKind'],
): IEdge => ({ source, target, siteEdgeKind });
...
describe('calculateGraphLayout', () => {
  it('preserves the depth-row layout for single-file sites', () => {
  ...
  it('wraps a crowded semantic depth into one labeled multi-row band', () => {
    ...
    const layout = calculateGraphLayout([
      ...
      ...linkedPages.map(linked => edge('contained', linked.siteNodeKey, 'semanticLink')),
    ]);

    const linkedPositions = linkedPages.map(linked => position(layout, linked.siteNodeKey));
