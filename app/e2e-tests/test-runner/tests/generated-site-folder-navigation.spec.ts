...
const NORMALIZATION_HOOK_SOURCE = `function pageTitleNormalization(siteSlug: string, pageTitle: string): string {
...
test("generated-site folder navigation uses normalized filenames and persists its UI state", async ({
  ...
  const folderNavigation = generatedSite.folderNavigation;
