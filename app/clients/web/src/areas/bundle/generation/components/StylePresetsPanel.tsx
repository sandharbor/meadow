...
interface PresetState {
  ...
  bundlePresetId: string | undefined;
  effectivePresetId: string;
  isInherited: boolean;
}

const StylePresetsPanel: React.FC<StylePresetsPanelProps> = ({ bundleSlug, onPresetChanged }) => {
  const [state, setState] = useState<PresetState>({
    presets: [],
    globalPresetId: 'classic',
    bundlePresetId: undefined,
  ...
  const loadPresets = useCallback(async () => {
    try {
      ...
      setState({
        ...
        bundlePresetId: bundleData.bundlePresetId,
        effectivePresetId: bundleData.effectivePresetId || 'classic',
        isInherited: bundleData.isInherited ?? true,
      });
    } catch (error) {
      ...
      if (scope === 'global') {
        ...
        if (response.ok) {
          ...
          setState(prev => ({
            ...
            // If bundle is inheriting, update effective preset too
        ...
        if (response.ok) {
          ...
          setState(prev => ({
            ...prev,
            bundlePresetId: data.bundlePresetId,
  ...
  const handleInheritToggle = async () => {
    if (state.isInherited) {
      // Switching to bundle-specific: select the current global preset
  ...
  const footer = (
    <>
      ...
      {state.isInherited && scope === 'bundle' && (
  ...
  return (
    ...
    >
      <div className="p-3">
        ...
        {/* Preset cards */}
        <div className={`grid grid-cols-1 sm:grid-cols-3 gap-3 ${scope === 'bundle' && state.isInherited ? 'opacity-50 pointer-events-none' : ''}`}>
          {state.presets.map(preset => {
            const isSelected = scope === 'global'
              ? state.globalPresetId === preset.id
              : !state.isInherited && state.bundlePresetId === preset.id;

            return (
              <button
                key={preset.id}
                onClick={() => handlePresetSelect(preset.id)}
                disabled={isSaving || (scope === 'bundle' && state.isInherited)}
