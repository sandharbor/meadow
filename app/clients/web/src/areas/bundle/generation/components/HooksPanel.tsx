...
const HooksPanel: React.FC<HooksPanelProps> = ({ bundleSlug, onHooksChanged }) => {
  ...
  const renderEditorToolbar = (editor: OpenEditor) => {
    if (editor.scope !== 'bundle') return undefined;
      ...
      <div className="text-sm">
        ...
        <div className="rounded border border-neutral-200">
          ...
          {HOOK_TYPES.map(({ type, label }) => {
            ...
            // Global cell display
            const globalFaded = row.bundleExists && !row.appendMode;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1 min-w-0">
                  ...
                  {(row.globalHasError || row.bundleHasError) && (
                ...
                <button
                  type="button"
                  onClick={() => openEditor(type, 'bundle')}
                  className="text-xs text-center cursor-pointer hover:bg-main-50 rounded py-0.5 transition-colors group/cell"
                >
                  {row.bundleExists ? (
                    row.appendMode ? (
