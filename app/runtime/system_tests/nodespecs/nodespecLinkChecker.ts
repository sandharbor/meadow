...
/**
 * Runtime link checking for nodespecs.
 * Validates that nodespec link specifications match actual working graph links.
 */

import type { NodespecLinkSpec, NodespecLinks } from './types.js';
import { IMAGE_EXTENSIONS } from '../../../shared_code/utils/fileTypeUtils.js';

const PRESERVED_LINK_EXTENSIONS = ['.html', '.css', '.js', '.pdf', ...IMAGE_EXTENSIONS] as const;
...
/**
 * Converts a nodespec link path to a page ID.
 * Link paths in nodespecs are like "/main page.md" or "/folder/page.md" for pages,
 ...
 *
 * @param linkPath - The link path from the nodespec (e.g., "/main page.md" or "/folder/image.png")
 ...
 *
 * @param specifiedLinks - Links specified in the nodespec
...
export function validateOutlinks(
  specifiedLinks: NodespecLinkSpec[],
  ...
  for (const spec of specifiedLinks) {
    ...
    if (!actualPaths.has(actualPath)) {
      errors.push({
        ...
        message: `${context}: Outlink "${spec.linkPath}" specified in nodespec but not found in actual outlinks`,
      });
      continue;
    }
  ...
  for (const actualPageId of actualLinkPageIds) {
    ...
    if (!specifiedPaths.has(actualPath)) {
      errors.push({
        ...
        linkType: 'outlink',
        linkPath: actualPath,
        message: `${context}: Actual outlink "${actualPath}" not specified in nodespec`,
      });
    }
  }
  ...
  for (const spec of specifiedLinks) {
    ...
    if (!actualPaths.has(actualPath)) {
      errors.push({
        ...
        linkType: 'inlink',
        linkPath: spec.linkPath,
        message: `${context}: Inlink "${spec.linkPath}" specified in nodespec but not found in actual inlinks`,
      });
      continue;
    }
  ...
  for (const actualPageId of actualLinkPageIds) {
    ...
    if (!specifiedPaths.has(actualPath)) {
      errors.push({
        ...
        linkType: 'inlink',
        linkPath: actualPath,
        message: `${context}: Actual inlink "${actualPath}" not specified in nodespec`,
      });
    }
  }
...
/**
 * Full validation of a nodespec's links against working graph data.
 *
 * @param links - The links section from the nodespec
 ...
 */
export function checkNodespecLinks(
  links: NodespecLinks,
  pageId: string,
  ...
  if (links.outlinks !== undefined) {
    ...
    for (const outlink of actualOutlinks) {
      errors.push({
        ...
        message: `${pageTitle}: Actual outlink "${pageIdToLinkPath(outlink)}" not specified in nodespec (outlinks section missing)`,
      });
    }
  }
  ...
  if (links.inlinks !== undefined) {
    ...
    for (const inlink of actualInlinks) {
      errors.push({
        ...
        message: `${pageTitle}: Actual inlink "${pageIdToLinkPath(inlink)}" not specified in nodespec (inlinks section missing)`,
      });
    }
  }
