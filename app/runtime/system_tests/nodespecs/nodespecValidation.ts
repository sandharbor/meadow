...
/**
 * Validation functions for nodespecs blocks.
 */
 ...
 */
const VALID_LINK_EXTENSIONS = ['.md', '.html', '.css', '.js', '.pdf', ...IMAGE_EXTENSIONS] as const;
...
/**
 * Validates a links section of a nodespec entry.
 *
...
/**
 * Validates an htmlRenderedLinks section of a nodespec entry.
 */
...
/**
 * Options for nodespec validation.
 */
...
/**
 * Validates a single nodespec entry.
 *
 * @param spec - The nodespec entry to validate
 ...
 * @returns Array of validation errors (empty if valid)
 */
export function validateNodespecEntry(
  spec: NodespecEntry,
...
/**
 * Validates a complete nodespecs block.
 *
 * @param block - The nodespecs block to validate
 * @param requiredBundles - Bundles that must have a nodespec entry
 * @param availableBundles - Set of valid bundle names
 * @param pageTitle - Title of the page containing this block (for error messages)
 * @param options - Optional validation options
 * @returns Array of validation errors (empty if valid)
 */
export function validateNodespecsBlock(
  block: NodespecsBlock,
  ...
  for (const spec of block.nodespecs) {
    errors.push(...validateNodespecEntry(spec, availableBundles, pageTitle, options));
  }
  ...
  for (const spec of block.nodespecs) {
    if (seenBundles.has(spec.bundle)) {
      errors.push({
        message: `Duplicate nodespec entry for bundle "${spec.bundle}"`,
        pageTitle,
        bundle: spec.bundle,
      });
    }
  ...
  const coveredBundles = new Set(block.nodespecs.map((s) => s.bundle));
  for (const requiredBundle of requiredBundles) {
    if (!coveredBundles.has(requiredBundle)) {
      errors.push({
        message: `Missing nodespec for required bundle "${requiredBundle}"`,
        pageTitle,
...
/**
 * Validates that a nodespecs block is well-formed (parseable and has required structure).
 *
 * @param block - The nodespecs block to validate
 * @param pageTitle - Title of the page containing this block (for error messages)
 * @returns Array of validation errors (empty if valid)
 */
export function validateNodespecsBlockStructure(
  block: unknown,
  pageTitle: string
): ValidationError[] {
  const errors: ValidationError[] = [];
  ...
  if (!block || typeof block !== 'object') {
    errors.push({
      message: 'Nodespecs block must be an object',
      pageTitle,
    });
    return errors;
  }
  ...
  if (!Array.isArray(blockObj.nodespecs)) {
    errors.push({
      message: 'Nodespecs block must have a "nodespecs" array',
      pageTitle,
    });
    return errors;
  }

  for (let i = 0; i < blockObj.nodespecs.length; i++) {
    const entry = blockObj.nodespecs[i] as Record<string, unknown>;

    if (!entry || typeof entry !== 'object') {
      errors.push({
        message: `Nodespec entry ${i} must be an object`,
        pageTitle,
      });
      continue;
    }
    ...
    if (typeof entry.bundle !== 'string') {
      errors.push({
        message: `Nodespec entry ${i} must have a "bundle" string`,
        pageTitle,
      });
    }
    ...
    if (!entry.generation || typeof entry.generation !== 'object') {
      errors.push({
        message: `Nodespec entry ${i} must have a "generation" object`,
        pageTitle,
      });
    }
    ...
    for (const key of Object.keys(entry)) {
      if (!allowedKeys.has(key)) {
        errors.push({
          message: `Nodespec entry ${i} has unknown key "${key}"`,
          pageTitle,
        });
      }
    }
    ...
    if (entry.curation && typeof entry.curation === 'object' && !Array.isArray(entry.curation)) {
      ...
      if (typeof curation.isTracked !== 'boolean') {
        errors.push({
          message: `Nodespec entry ${i}.curation must have an "isTracked" boolean`,
          pageTitle,
        });
      }
      ...
      for (const key of Object.keys(curation)) {
        if (!allowedCurationKeys.has(key)) {
          errors.push({
            message: `Nodespec entry ${i}.curation has unknown key "${key}"`,
            pageTitle,
          });
        }
      }
    ...
    if (entry.generation && typeof entry.generation === 'object') {
      ...
      for (const key of Object.keys(generation)) {
        if (!allowedGenerationKeys.has(key)) {
          errors.push({
            message: `Nodespec entry ${i}.generation has unknown key "${key}"`,
            pageTitle,
          });
        }
      }
