...
/**
 * Generates paired node spec files for all Markdown files in
 * example-bundle-data. Native HTML and SVG nodes keep their NodeSpecs in
 * sidecar files and are not modified by this script.
 *
 * Usage:  npx tsx runtime/system_tests/scripts/generate-example-nodespecs.ts
 *
 * This script:
 ...
 *  5. Computes correct nodespecs for every markdown file
 *  6. Writes each paired .nodespec.yaml file
...
async function main() {
  ...
  const setup = new SystemTestBundleSetup('home_fixture_example', 'generate-nodespecs', {
  ...
  try {
    ...
    for (const srcFile of sourceFiles) {
      ...
      // Write expectations beside the unmodified source file.

      const yamlStr = YAML.stringify({ nodespecs: [entry] }, {
        lineWidth: 0,        // no line wrapping
        defaultKeyType: 'PLAIN',
        defaultStringType: 'PLAIN',
      }).trimEnd();

      fs.writeFileSync(getSidecarNodespecPath(srcFile), yamlStr + '\n', 'utf-8');
    }

    console.log('Done! All nodespecs updated.');
  } finally {
    setup.tearDown();
    forceStopServer();
  }
